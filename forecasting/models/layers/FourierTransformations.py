...
Fourier blocks are used in fedformer architecture

Code inspired from: https://github.com/MAZiqing/FEDformer
"""
class FourierBlock(nn.Module):
    def __init__(self, in_channels, out_channels, seq_len, modes=0, mode_select_method='random'):
        super(FourierBlock, self).__init__()
        """
        Fourier enhancement block
        """
        #get modes
        ...
        self.weights1 = nn.Parameter(
            self.scale * torch.rand(4, in_channels // 4, out_channels // 4, len(self.index), dtype=torch.float))
...
class FourierCrossAttention(nn.Module):
    def __init__(self, in_channels, out_channels, seq_len_q, seq_len_kv, modes=64, mode_select_method='random',
        ...
        self.scale = (1 / (in_channels * out_channels))
        self.weights1 = nn.Parameter(
            self.scale * torch.rand(4, in_channels // 4, out_channels // 4, len(self.index_q), dtype=torch.float))
        self.weights2 = nn.Parameter(
            self.scale * torch.rand(4, in_channels // 4, out_channels // 4, len(self.index_q), dtype=torch.float))
    ...
    def forward(self, q, k, v, mask):
        ...
        # perform computations on frequency domain
...
def get_frequency_modes(seq_len, modes=64, mode_select_method='random'):
    """
    Get frequency modes (standard use random for best performance)
    """
